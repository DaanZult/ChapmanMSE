...
est_sds = numeric()
for (s in 1:length(scenarios)){
  est_sd  = format(round(colSds(results_scenarios[[s]][,c(1,2,3,4,7,8)],na.rm=TRUE),1),nsmall=1)
  est_sds = cbind(est_sds, est_sd)
}
est_sds = est_sds[-1,]
...
est_rmses = numeric()
for (s in 1:length(scenarios)){
  est_rmse  = format(round(colMeans(((results_scenarios[[s]][,c(1,2,3,4,7,8)]-scenarios[[s]]$N)^2),na.rm=TRUE)^0.5,1),nsmall=1)
